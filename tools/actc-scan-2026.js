...
const ctcConfig = yaml.load(fs.readFileSync(path.join(process.cwd(), 'jurisdictions/us-federal/2026/credits/ctc.yaml'), 'utf8'));
...
for (const file of files) {
  ...
  if (children === 0 || !Object.prototype.hasOwnProperty.call(outputs, 'us-1040-2026/line-28')) continue;

  const preCreditTax = outputs['us-1040-2026/line-16'] ?? 0;
  const eitc = outputs['us-1040-2026/line-27'] ?? 0;
  const withheld = outputs['us-1040-2026/line-25a'] ?? 0;
  const excessSS = outputs['us-1040-s3-2026/line-11'] ?? 0;
  const aotcRefundable = outputs['us-1040-2026/line-29'] ?? 0;
  ...
  if (Math.abs(correctLine28 - (outputs['us-1040-2026/line-28'] ?? 0)) > 1) {
    console.log(`FIXING ${file}: line-28 ${outputs['us-1040-2026/line-28']} -> ${correctLine28}`);
    outputs['us-1040-2026/line-28'] = correctLine28;
    outputs['us-1040-2026/line-24'] = totalTax;
    outputs['us-1040-2026/line-34'] = totalPayments;
    outputs['us-1040-2026/line-35a'] = refundOrOwe > 0 ? refundOrOwe : 0;
    outputs['us-1040-2026/line-37'] = refundOrOwe < 0 ? Math.abs(refundOrOwe) : 0;
